...
describe("Rooms API", () => {
  ...
  test("POST /api/rooms - Creates a new room", async () => {
    ...
    const response = await request(app).post("/api/rooms").set("Cookie", [...headers["set-cookie"]]).send(roomData);
    roomId = response.body.roomId
    expect(response.status).toEqual(200);
    });

  // Test case for GET /api/rooms/:id
  test("GET /api/rooms/:id - Retrieves a specific room", async () => {
    const response = await request(app).get(`/api/rooms/${roomId}`).set("Cookie", [...headers["set-cookie"]]);
    expect(response.status).toEqual(200);
  ...
  test("GET /api/rooms/name/:id - Retrieves name of a specific room", async () => {
    const response = await request(app).get(`/api/rooms/name/${roomId}`).set("Cookie", [...headers["set-cookie"]]);
    expect(response.status).toEqual(200);
  ...
  test("DELETE /api/rooms/:id - Deletes a specific room", async () => {
    const response = await request(app).delete(`/api/rooms/${roomId}`).set("Cookie", [...headers["set-cookie"]]);
